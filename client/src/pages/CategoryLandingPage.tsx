...
const CategoryLandingPage: React.FC<CategoryLandingPageProps> = ({ category }) => {
    ...
    const fetchSections = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`${API_URL}/sections?page=${category}`);
            if (response.data.success) {
                setSections(response.data.data.sections);
            }
        } catch (error) {
            console.error('Failed to fetch sections:', error);
            showToast('Failed to load category sections', 'error');
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    <h2 className="text-2xl font-bold mb-2 capitalize">{category}'s Collection</h2>
                    <p className="text-secondary mb-4">Explore our latest collection</p>
                    <a
